...
import { resendOtp, verifyOtp } from "../../../api/auth/userAuth";
import type { OTPVerificationResponse } from "../interface/IAuthPage";
...
const OTPVerification = () => {
  ...
  const handleResend = async () => {
    const email = localStorage.getItem("email") || localStorage.getItem("ForgotPassEmail");

    console.log("email stored in the local storage",email)
    if (!email) {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response: OTPVerificationResponse = await verifyOtp(code);
      if (response.success) {
        ...
        setTimeout(() => {
          const redirectTo = localStorage.getItem("ForgotPassEmail")
            ? "/user/resetPassword"
            : "/user/login";
          navigate(redirectTo);
        }, 1500);
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-black/80 backdrop-blur-xl rounded-lg p-10 md:p-14 border border-gray-800 shadow-2xl text-center">
          ...
          <div className="mb-10">
            ...
            <p className="text-gray-400 mt-4 text-lg">Verify Your Email</p>
          </div>

          <h2 className="text-3xl font-bold mb-4">Enter Verification Code</h2>
